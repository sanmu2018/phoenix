...
export const CodeEvaluatorTestSection = ({
  ...
  // Session keying for inline previews is now derived server-side from the
  // caller's auth identity plus (sandbox_config_id, language). The frontend
  // no longer mints or echoes a session id, and the matching
  // ``stopEvaluatorSession`` mutation has been removed — eviction is the
  // manager's idle TTL, which the docstring on the mutation already called
  // out as the load-bearing path.
