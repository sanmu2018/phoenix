...
async def test_traces_and_spans_sequence_cycle(
    ...
    assert sorted(trace_ids) == await _get_all_trace_ids(_engine)
    assert sorted(span_ids) == await _get_all_span_ids(_engine)

    # After downgrade, sequences restart at 1
    new_trace_id = await _insert_trace(_engine, project_id)
    assert new_trace_id == 1
    new_span_id = await _insert_span(_engine, new_trace_id)
    assert new_span_id == 1
